...
# Comprehensive unit tests for document linkage and lineage functions
...
class TestLinkDocuments:
    ...
    def test_empty_documents_list(self):
        ...
        documents = []
        link_documents(documents)  # Should not raise
        assert documents == []
...
class TestDataQualityLinkage:
    ...
    def test_link_documents_with_real_structure(self):
        ...
        # Simulate real document structure from load_all_documents
...
@pytest.mark.skipif(not DATA_DIR.exists(), reason="Data directory not available")
class TestDataQualityLineageCache:
    """Data quality tests for lineage cache with real documents."""

    def test_lineage_cache_coverage(self):
        """All PDFs should be processable for lineage cache."""
        from mandate_pipeline.extractor import extract_text

        pdfs = list(DATA_DIR.glob("*.pdf"))[:20]
        if len(pdfs) < 10:
            pytest.skip("Not enough PDFs")
        ...
        for pdf in pdfs:
            try:
                ...
                assert classification in ("proposal", "resolution", "other")
            except Exception as e:
                errors.append(f"{pdf.name}: {e}")

        assert len(errors) == 0, f"Errors processing: {errors}"

    def test_existing_lineage_cache_valid(self):
        """Existing lineage.json cache should be valid."""
        cache_path = DATA_DIR.parent / "lineage.json"
        if not cache_path.exists():
            pytest.skip("lineage.json not available")

        cache = load_lineage_cache(cache_path)

        assert "documents" in cache
        assert isinstance(cache["documents"], dict)

        # Validate structure of cached documents
        for symbol, doc_data in list(cache["documents"].items())[:10]:
            assert "classification" in doc_data
            assert "links" in doc_data
            assert doc_data["classification"] in ("proposal", "resolution", "other")
            assert isinstance(doc_data["links"], list)
...
class TestLinkageIntegration:
    ...
    def test_visible_documents_filter(self):
        """Adopted drafts should be filtered from visible documents."""
        documents = [
            {
                "symbol": "A/RES/80/1",
                "doc_type": "resolution",
        ...
        # Filter visible documents (mimics static_generator.py logic)
        visible = [d for d in documents if not d.get("is_adopted_draft")]

        assert len(visible) == 1
        assert visible[0]["symbol"] == "A/RES/80/1"
...
class TestLinkDocumentsEdgeCases:
    """Edge case tests for link_documents function."""

    def test_missing_title_field(self):
        """Handle documents without title field."""
        documents = [
            {
                "symbol": "A/RES/80/1",
                "agenda_items": ["Item 68"],
        ...
        # Resolution without title should not fuzzy match
        resolution = documents[0]
        assert resolution["linked_proposal_symbols"] == []

    def test_none_title_field(self):
        """Handle documents with None title."""
        documents = [
            {
                "symbol": "A/RES/80/1",
                "title": None,
    ...
    def test_empty_title_field(self):
        """Handle documents with empty string title."""
        documents = [
            {
                "symbol": "A/RES/80/1",
                "title": "",
    ...
    def test_none_agenda_items(self):
        """Handle None agenda_items field."""
        documents = [
            {
                "symbol": "A/RES/80/1",
                "title": "Climate Action",
        ...
        # Should still link via fuzzy title match
        resolution = documents[0]
        assert resolution["linked_proposal_symbols"] == ["A/80/L.1"]

    def test_missing_symbol_references(self):
        """Handle missing symbol_references field."""
        documents = [
            {
                "symbol": "A/RES/80/1",
                "title": "Climate Action",
                "agenda_items": ["Item 68"],
                # No symbol_references field
            },
            {
                "symbol": "A/80/L.1",
                "title": "Climate Action",
    ...
    def test_confidence_boost_with_agenda_overlap(self):
        """Verify confidence boosted when agenda items overlap."""
        documents = [
            {
                "symbol": "A/RES/80/1",
                "title": "Climate Action",
            ...
            },
            {
                "symbol": "A/80/L.1",
                "title": "Climate Action",
                "agenda_items": ["Item 68"],  # Same agenda item
                "symbol_references": [],
            },
        ]
        ...
        resolution = documents[0]
        # Confidence should be boosted by 0.05 for agenda overlap
        assert resolution["link_confidence"] > 0.85

    def test_best_match_selection(self):
        """Select best fuzzy match when multiple candidates exist."""
        documents = [
            {
                "symbol": "A/RES/80/1",
                "title": "Strengthening humanitarian assistance",
            ...
            {
                "symbol": "A/80/L.1",
                "title": "Strengthening humanitarian assistance programs",  # 90% similar
                "agenda_items": ["Item 68"],
                "symbol_references": [],
            },
            {
                "symbol": "A/80/L.5",
                "title": "Strengthening humanitarian assistance",  # 100% similar
                "agenda_items": ["Item 68"],
                "symbol_references": [],
            },
        ]
        ...
        resolution = documents[0]
        # Should select L.5 as the best match
        assert resolution["linked_proposal_symbols"] == ["A/80/L.5"]

    def test_committee_proposal_linking(self):
        """Link resolution to committee proposal."""
        documents = [
            {
                "symbol": "A/RES/80/1",
                "title": "Climate Action",
                "agenda_items": ["Item 68"],
                "symbol_references": ["A/C.2/80/L.5"],
            },
            {
                "symbol": "A/C.2/80/L.5",
                "title": "Climate Action",
        ...
        assert resolution["linked_proposal_symbols"] == ["A/C.2/80/L.5"]
        assert proposal["linked_resolution_symbol"] == "A/RES/80/1"
    ...
    def test_special_characters_in_title(self):
        ...
        documents = [
            {
                "symbol": "A/RES/80/1",
                "title": "80/1. Question of Palestine: two-State solution",
                "agenda_items": ["Item 35"],
                "symbol_references": [],
            },
            {
                "symbol": "A/80/L.1",
                "title": "Question of Palestine: two-State solution",
                "agenda_items": ["Item 35"],
                "symbol_references": [],
            },
        ]
    ...
    def test_non_proposal_not_linked(self):
        """Non-proposal documents should not be linked as proposals."""
        documents = [
            {
                "symbol": "A/RES/80/1",
                "title": "Climate Action",
                "agenda_items": ["Item 68"],
                "symbol_references": ["A/80/100"],  # A report, not a proposal
            },
            {
                "symbol": "A/80/100",  # This is a report (no /L.)
        ...
        resolution = documents[0]
        # Should not link to A/80/100 (it's not a proposal)
        assert resolution["linked_proposal_symbols"] == []


class TestAnnotateLineageEdgeCases:
    """Edge case tests for annotate_lineage function."""

    def test_missing_linked_resolution_symbol(self):
        """Handle missing linked_resolution_symbol field."""
        documents = [
            {
                "symbol": "A/80/L.1",
                "doc_type": "proposal",
                # No linked_resolution_symbol field
            },
        ]
        ...
        doc = documents[0]
        assert doc["is_adopted_draft"] is False

    def test_amendment_doc_type(self):
        """Amendment doc_type should not be marked as adopted base."""
        documents = [
            {
                "symbol": "A/80/L.1",
                "doc_type": "amendment",
    ...
    def test_lineage_with_unknown_proposal(self):
        """Handle linked_proposal_symbols pointing to unknown documents."""
        documents = [
            {
                "symbol": "A/RES/80/1",
                "doc_type": "resolution",
                "linked_proposal_symbols": ["A/80/L.999"],  # Doesn't exist
            },
        ]
        ...
        resolution = documents[0]
        # Should have empty lineage_proposals (unknown symbol filtered)
        assert resolution["lineage_proposals"] == []


# =============================================================================
# DATA QUALITY TESTS: Real Document Linkage
# =============================================================================


@pytest.mark.skipif(not DATA_DIR.exists(), reason="Data directory not available")
class TestRealDocumentLinkage:
    """Data quality tests for linkage with real resolution-proposal pairs."""

    def test_resolution_proposal_pairs(self):
        """Test linkage between real resolutions and proposals."""
        from mandate_pipeline.extractor import extract_text, extract_title, extract_agenda_items, find_symbol_references

        # Check if we have both resolutions and proposals
        res_files = list(DATA_DIR.glob("A_RES_80_*.pdf"))
        proposal_files = list(DATA_DIR.glob("A_80_L.*.pdf"))

        if len(res_files) < 3 or len(proposal_files) < 5:
            pytest.skip("Not enough resolution/proposal pairs")

        documents = []

        # Load some resolutions
        for pdf in res_files[:5]:
            text = extract_text(pdf)
            symbol = filename_to_symbol(pdf.stem)
            documents.append({
                "symbol": symbol,
        ...
        # Verify all resolutions have link fields
        resolutions = [d for d in documents if is_resolution(d["symbol"])]
        for res in resolutions:
            assert "linked_proposal_symbols" in res
            assert "link_method" in res

        # Verify all proposals have adoption fields
        proposals = [d for d in documents if is_proposal(d["symbol"])]
        for prop in proposals:
            assert "is_adopted_draft" in prop
            assert "adopted_by" in prop

    def test_symbol_reference_accuracy(self):
        """Verify symbol references are accurately extracted from real docs."""
        from mandate_pipeline.extractor import extract_text

        # Test with a resolution that should have references
        res_files = list(DATA_DIR.glob("A_RES_80_*.pdf"))[:5]
        if not res_files:
            pytest.skip("No resolution PDFs available")

        for pdf in res_files:
            text = extract_text(pdf)
            symbol = filename_to_symbol(pdf.stem)
            links = extract_linked_symbols(text, symbol)

            # Links should be valid symbols
            for link in links:
                assert "/" in link, f"Invalid symbol format: {link}"
                assert link == link.upper(), f"Symbol not normalized: {link}"

    def test_committee_document_linkage(self):
        """Test linkage for committee documents."""
        from mandate_pipeline.extractor import extract_text, extract_title, extract_agenda_items, find_symbol_references

        committee_files = list(DATA_DIR.glob("A_C.*_80_L.*.pdf"))[:10]
        if len(committee_files) < 5:
            pytest.skip("Not enough committee documents")

        documents = []
        for pdf in committee_files:
            text = extract_text(pdf)
            symbol = filename_to_symbol(pdf.stem)
            documents.append({
                "symbol": symbol,
        ...
        # All committee proposals should have link fields initialized
        for doc in documents:
            assert "linked_resolution_symbol" in doc
            assert "is_adopted_draft" in doc

    def test_lineage_cache_roundtrip(self, tmp_path):
        """Test that lineage cache can be updated and reloaded."""
        from mandate_pipeline.extractor import extract_text
        ...
        # Create temp data directory with copies
        temp_data = tmp_path / "data"
        temp_pdfs = temp_data / "pdfs"
        temp_pdfs.mkdir(parents=True)

        for pdf in pdfs:
            import shutil
            shutil.copy(pdf, temp_pdfs / pdf.name)

        # First update
        result1 = update_lineage_cache(temp_data)
        assert result1["updated"] == len(pdfs)
        assert result1["reused"] == 0

        # Second update (should reuse)
        result2 = update_lineage_cache(temp_data)
        assert result2["updated"] == 0
        assert result2["reused"] == len(pdfs)

        # Verify cache contents
        cache = load_lineage_cache(temp_data / "lineage.json")
        assert len(cache["documents"]) == len(pdfs)
...
class TestLinkageRegressions:
    """Regression tests for specific bugs or edge cases found in production."""

    def test_preserve_existing_base_proposal_symbol(self):
        """Don't overwrite existing base_proposal_symbol."""
        documents = [
            {
                "symbol": "A/RES/80/1",
                "title": "Climate Action",
                "agenda_items": ["Item 68"],
                "symbol_references": ["A/80/L.1"],
                "base_proposal_symbol": "A/80/L.ORIGINAL",  # Pre-existing
            },
            {
                "symbol": "A/80/L.1",
                "title": "Climate Action",
        ...
        resolution = documents[0]
        # Should preserve the original base_proposal_symbol
        assert resolution["base_proposal_symbol"] == "A/80/L.ORIGINAL"

    def test_proposal_reference_to_self_revision(self):
        """Handle proposal that references its own revision."""
        documents = [
            {
                "symbol": "A/80/L.1",
                "title": "Climate Action",
                "agenda_items": ["Item 68"],
                "symbol_references": ["A/80/L.1/Rev.1"],
            },
            {
                "symbol": "A/80/L.1/Rev.1",
                "title": "Climate Action Revised",
                "agenda_items": ["Item 68"],
                "symbol_references": [],
            },
        ]
        ...
        # Both should exist and have fields
        for doc in documents:
            assert "linked_resolution_symbol" in doc

    def test_resolution_with_only_non_matching_proposals(self):
        """Resolution with proposals but none matching (different agenda)."""
        documents = [
            {
                "symbol": "A/RES/80/1",
                "title": "Climate Action",
            ...
            },
            {
                "symbol": "A/80/L.1",
                "title": "Climate Action",
                "agenda_items": ["Item 999"],  # Different agenda
                "symbol_references": [],
            },
            {
                "symbol": "A/80/L.5",
                "title": "Climate Action",
                "agenda_items": ["Item 888"],  # Different agenda
                "symbol_references": [],
            },
        ]
    ...
    def test_multiple_resolutions_same_proposal(self):
        """Multiple resolutions referencing the same proposal."""
        documents = [
            {
                "symbol": "A/RES/80/1",
                "title": "Climate Action",
            ...
            {
                "symbol": "A/RES/80/2",
                "title": "Climate Action Extension",
                "agenda_items": ["Item 68"],
                "symbol_references": ["A/80/L.1"],  # Same proposal
            },
            {
                "symbol": "A/80/L.1",
                "title": "Climate Action",
        ...
        # Both resolutions should link to the proposal
        res1 = documents[0]
        res2 = documents[1]
        assert res1["linked_proposal_symbols"] == ["A/80/L.1"]
        assert res2["linked_proposal_symbols"] == ["A/80/L.1"]

        # Proposal should link to first resolution that claims it
        proposal = documents[2]
        assert proposal["linked_resolution_symbol"] == "A/RES/80/1"
